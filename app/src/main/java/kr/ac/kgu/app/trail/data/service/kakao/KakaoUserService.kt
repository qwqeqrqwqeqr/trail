    ...
    override suspend fun loginWithKakaoAccount() {
        if (UserApiClient.instance.isKakaoTalkLoginAvailable(context)) {
            UserApiClient.instance.loginWithKakaoTalk(context) { token, error ->
                if (error != null) {
                    Log.e(Constants.KAKAO, "카카오톡으로 로그인 실패", error)
                    ...
                    Log.i(Constants.KAKAO, "카카오톡으로 로그인 성공 ${token.accessToken}")
                }
            }
        } else {
...
//    }
    ...
    override suspend fun accessTokenInfo() {
        ...
        UserApiClient.instance.accessTokenInfo { tokenInfo, error ->
            if (error != null) {
                Log.e(Constants.KAKAO, "토큰 정보 보기 실패", error)
            } else if (tokenInfo != null) {
                Log.i(
                    Constants.KAKAO, "토큰 정보 보기 성공" +
                            "\n회원번호: ${tokenInfo.id}" +
    ...
    override suspend fun unlink() {
        UserApiClient.instance.unlink { error ->
            if (error != null) {
                Log.e(Constants.KAKAO, "연결 끊기 실패", error)
            } else {
                Log.i(Constants.KAKAO, "연결 끊기 성공. SDK에서 토큰 삭제 됨")
            }
        }
    }
    ...
    override suspend fun logout() {
        UserApiClient.instance.logout { error ->
            if (error != null) {
                Log.e(Constants.KAKAO, "로그아웃 실패. SDK에서 토큰 삭제됨", error)
            } else {
                Log.i(Constants.KAKAO, "로그아웃 성공. SDK에서 토큰 삭제됨")
            }
        }
    }
